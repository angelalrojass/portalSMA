  ...
  class Users
  {
      ...
      private function loginCheck()
      {
          if (isset($_SESSION['username']) && $_SESSION['username'] != "Guest") {
			  ...
			  $this->nombre = $row->fname;
			  $this->apellido_uno = $row->lname;
			  $this->apellido_dos = $row->sname; 
	  ...
	  public function login($username, $pass)
	  {
		  ...
		  if ($username == "" && $pass == "") {
			  Filter::$msgs['username'] = 'Usuario o Contraseña incorrectos.';
		  } else {
			  $status = $this->checkStatus($username, $pass);
			  
              switch ($status) {
                  case 0:
                      Filter::$msgs['username'] = 'No tenemos este Usuario y/o Contraseña registrados en el sistema.';
                      break;

                  case 1:
                      Filter::$msgs['username'] = 'Tu cuenta ha sido deshabilitada.';
                      break;

                  case 2:
                      Filter::$msgs['username'] = 'Tu cuenta no esta activa.';
                      break;

                  case 3:
                      Filter::$msgs['username'] = 'Necesitas verificar tu Email.';
                      break;
              }
		  }
          if (empty(Filter::$msgs) && $status == 5) {
			  ...
			  $this->nombre = $_SESSION['nombre'] = $row->fname;
			  $this->apellido_uno = $_SESSION['apellido_uno'] = $row->lname;
			  $this->apellido_dos = $_SESSION['apellido_dos'] = $row->sname; 
      ...
      public function logout()
      {
		  ...
		  unset($_SESSION['apellido_uno']);
		  unset($_SESSION['apellido_dos']);
	  ...
	  public function processUser()
	  {
		  ...
		  if (!Filter::$id) {
			  ...
			  if ($value = $this->usernameExists($_POST['username'])) {
				  if ($value == 1)
					  Filter::$msgs['username'] = 'Nombre de usuario muy corto (debe contener al menos 6 caracteres).';
				  if ($value == 2)
					  Filter::$msgs['username'] = 'Sólo se permite letras y números en el campo de usuario.';
				  if ($value == 3)
					  Filter::$msgs['username'] = 'Lo sentimos este nombre de Usuario ya esta en uso';
			  }
		  }

		  if (empty($_POST['fname']))
			  Filter::$msgs['fname'] = 'Por favor ingresa tu nombre';
			  
		  if (empty($_POST['lname']))
			  Filter::$msgs['lname'] = 'Por favor ingrsa tu Primer Apellido';
			  
		  if (!Filter::$id) {
			  if (empty($_POST['password']))
				  Filter::$msgs['password'] = 'Ingresa una contraseña válida.';
		  }

		  if (empty($_POST['email']))
			  Filter::$msgs['email'] = 'Por favor ingresa una dirección de Email válida';
		  if (!Filter::$id) {
			  if ($this->emailExists($_POST['email']))
				  Filter::$msgs['email'] = 'El Email que ingresaste ya esta en uso por otro usuario.';
		  }
		  if (!$this->isValidEmail($_POST['email']))
			  Filter::$msgs['email'] = 'La dirección de Email que ingresaste no es válida.';

          if (!empty($_FILES['avatar']['name'])) {
              if (!preg_match("/(\.jpg|\.png)$/i", $_FILES['avatar']['name'])) {
                  Filter::$msgs['avatar'] = "Tipo de archivo no permitido. Sólo se permiten archivos con extensión jpg.";
              }
		  ...
		  if (empty(Filter::$msgs)) {
              ...
              $message = (Filter::$id) ? '<span>Listo!</span> Expediente actualizado!' : '<span>Listo!</span> Usuario agregado correctamente!';
              ...
              if (self::$db->affected()) {
                  ...
                  Filter::msgAlert('<span>Aviso! </span>Nada que procesar.');
          } else
	  ...
	  {		  
		  if (empty($_POST['username']))
			  Filter::$msgs['username'] = '<span class="alert-danger">Utiliza un nombre de usuario válido</span>';
		  
		  if ($value = $this->usernameExists($_POST['username'])) {
			  if ($value == 1)
				  Filter::$msgs['username'] = '<span class="alert-danger">Nombre de usuario muy corto (Débe contener mínimo 4 caracteres).</span>';
			  if ($value == 2)
				  Filter::$msgs['username'] = '<span class="alert-danger">El nombre de usurio solo puede contener letras y números.</span>';
			  if ($value == 3)
				  Filter::$msgs['username'] = '<span class="alert-danger">Lo sentimo, este nombre de usuario ya esta en uso</span>';
		  }

		  if (empty($_POST['fname']))
			  Filter::$msgs['fname'] = '<span class="alert-danger">Por favor ingresa tu Nombre</span>';
			  
		  if (empty($_POST['lname']))
			  Filter::$msgs['lname'] = '<span class="alert-danger">por favor ingresa Primer Apellido</span>';
			  
		  if (empty($_POST['pass']))
			  $this->msgs['pass'] = '<span class="alert-danger">Ingresa una contraseña válida.</span>';
		  
		  if (strlen($_POST['pass']) < 6)
			  Filter::$msgs['pass'] = '<span class="alert-danger">La contraseña es muy corta (Débe contener mínimo 6 caracteres)</span>';
		  elseif (!preg_match("/^[a-z0-9_-]{6,15}$/", ($_POST['pass'] = trim($_POST['pass']))))
			  Filter::$msgs['pass'] = '<span class="alert-danger">La contraseña que ingresaste no es alfanumérica.</span>';
		  elseif ($_POST['pass'] != $_POST['pass2'])
			  Filter::$msgs['pass'] = '<span class="alert-danger">La contraseña no coincide!.</span>';
		  
		  if (empty($_POST['email']))
			  Filter::$msgs['email'] = '<span class="alert-danger">Ingresa una dirección de email válida</span>';
		  
		  if ($this->emailExists($_POST['email']))
			  Filter::$msgs['email'] = '<span class="alert-danger">Este email ya esta registrado por otro usuario.</span>';
		  
		  if (!$this->isValidEmail($_POST['email']))
			  Filter::$msgs['email'] = '<span class="alert-danger">La dirección de email que ingresaste no es válida.</span>';
			  		  
		  if (empty($_POST['captcha']))
			  Filter::$msgs['captcha'] = '<span class="alert-danger">Por favor capturo el código de verificación!</span>';
		  
		  if ($_SESSION['captchacode'] != $_POST['captcha'])
			  Filter::$msgs['captcha'] = '<span class="alert-danger">El codigo de verificación es incorrecto</span>';
		  ...
		  if (empty(Filter::$msgs)) {
              ...
              (self::$db->affected() && $mailer) ? print "OK" : Filter::msgError('<span>Error!</span> Hubo un error durante el proceso de registro. Ponte en contato con el administrador...', false);
          } else
              print Filter::msgStatus();
      }
     ...
     private function validateToken($token)
      ...
      public function getUserLevels($level = false)
	  {
		  $arr = array(
				 ...
				 6 => 'Salud Pública',
				 7 => 'Tesorería',
				 8 => 'Dir Protección Civil',
				 10 => 'Dir Medio Ambiente',
				 11 => 'Dir Desarrollo Urbano',
				 12 => 'Dir Salud'
		  );
