...
if ($result = mysqli_query($link, "SELECT * FROM codigos_postales WHERE CodPos='$CodPostal'")) {
    ...
    $row_cnt = mysqli_num_rows($result);
		 ...
		 echo '	<label for="Colonia" class="col-sm-4 control-label">Colonia <span class="text-danger">*</span></label>';
		 echo '	<div class="col-sm-8">';
		 echo '<select class="form-control mb-10" id="Colonia" name="Colonia" tabindex="18" required>';
	...
	while($row = mysqli_fetch_assoc($result)) {
		...
		echo '		<input type="text" class="form-control" id="municipio" name="municipio" value="'.$municipio.'" disabled>';
		echo '	</div>';
		echo '</div>';
		echo '<div class="form-group">';
		...
		echo '		<input type="text" class="form-control" id="estado" name="estado" value="'.$estado.'" disabled>';
		echo '	</div>';
		echo '</div>';
