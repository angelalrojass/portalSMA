...
require_once('../../lib/pdf/tcpdf_config_alt.php');
...
$anuncio = sanitize($_POST['anuncio']);
$tipoAnuncio = sanitize($_POST['tipoAnuncio']);
$leyenda = sanitize($_POST['leyenda']);
$dimAnuncio = sanitize($_POST['dimAnuncio']);
...
/// Generamos contraseña Ramdom
$passV = '';
  for($x = 0; $x<1; $x++) {
	  $passV .= '-'.substr(strtoupper(sha1(rand(0,999999999999999))),2,8);
  }
  $passV = substr($passV,1);
  
///Insertamos usuario 
$creaUser = "INSERT INTO `usuarios`(`nombreu`, `apellidopu`, `apellidomu`, `email`, `password`, `calle`, `numero`, `colonia`, `codigoPostal`, `telefonoParticular`, `telefonoTrabajo`, `celular`,  `ip`, `activo`, `tipousuario`) VALUES ('$Nombre', '$ApellidoPat', '$ApellidoMat', '$EmailF', md5('$passV'), '$Calle', '$NumExt', '$Colonia', '$CodPostal', '$TelParticular', '$TelNego', '$TelMovil', '$ipdir', '1', '1')";
$usuarioCreado = $conexion -> query($creaUser);

//Obtenemos ultimo ID para insertarlo en la Solicitud
$rs = "SELECT `usuarioid` FROM `usuarios` ORDER BY `usuarioid` DESC LIMIT 1";
$resID = mysqli_query($conexion, $rs);
if($resID) {
	while($row = mysqli_fetch_assoc($resIDS)) {
		$idUserN = $row['usuarioid'];
	}
...
$guarda = "INSERT INTO `solicitudes`(`usuarioid`, `GiroComercial`, `Pfm`, `TipoUsoLic`, `Folio`, `Nombre`, `ApellidoPat`, `ApellidoMat`, `Rfc`, `factura`,  `TelParticular`, `TelMovil`, `TelNego`, `Email`, `Calle`, `NumInt`, `NumExt`, `CodPostal`, `Colonia`, `EntreCalles`, `YCalle`, `domfis`, `repleg`, `RazSocE`, `NomComE`, `CalleFis`, `NumExtFis`, `NumIntFis`, `CodPostalFis`, `ColoniaFis`, `NombreFis`, `ApellidoPatFis`, `ApellidoMatFis`, `SupMts`, `NumLevels`, `CajEsta`, `CveCat`, `ProRen`, `InvEst`, `NumEmp`, `EmpDis`, `EmpDisNo`, `anuncio`, `tipoAnuncio`, `leyenda`, `dimAnuncio`, `DocIne`, `DocCompDom`, `DocFoto1F`, `DocFoto2F`, `DocFoto1Int`, `DocFoto2Int`, `DocActaCons`, `croquis_loc`, `Declaraciones`, `IpUser`, `capturo`) VALUES ('$idUserN', '$GiroComercial', '$pfm', '$usolic', '$Folio','$Nombre', '$ApellidoPat', '$ApellidoMat', '$rfc', '$qf', '$TelParticular', '$TelMovil', '$TelNego', '$EmailF', '$Calle', '$NumInt', '$NumExt', '$CodPostal', '$Colonia', '$EntreCalles', '$YCalle', '$domfis', '$repleg', '$RazSocE', '$NomComE', '$CalleFis', '$NumExtFis', '$NumIntFis', '$CodPostalFis', '$ColoniaFis', '$NombreFis', '$ApellidoPatFis', '$ApellidoMatFis', '$SupMts', '$NumLevels', '$CajEsta', '$CveCat', '$proren', '$InvEst', '$NumEmp', '$EmpDis', '$EmpDisNo', '$anuncio', '$tipoAnuncio', '$leyenda', '$dimAnuncio', '$DocIne', '$DocCompDom', '$DocFoto1F', '$DocFoto2F', '$DocFoto1Int', '$DocFoto2Int', '$DocActaCons', '$imageName', '$Declaraciones', '$ipdir', '2')";
...
class MYPDF extends TCPDF {
	...
	public function Header() {
		...
		$this->ImageSVG($file='../../lib/svg/LogoMunicipio.svg', $x=0, $y=5, $w='50', $h='35', $link='', $align='', $palign='', $border=0, $fitonpage=false);
		
		$this->ImageSVG($file='../../lib/svg/LogoSanMateoVertical.svg', $x=159, $y=5, $w='47', $h='34', $link='', $align='', $palign='', $border=0, $fitonpage=false);
	    
		$this->ImageSVG($file='../../lib/svg/LogotipoSare.svg', $x=80, $y=4, $w='50', $h='35', $link='', $align='', $palign='', $border=0, $fitonpage=false);
		// Set font
...
$html = '<table width="700" border="0" cellspacing="0" cellpadding="0" style="font-family:Arial, Helvetica, sans-serif; font-size:10px;">
      ...
      <tr>
        <td height="25" align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:10px; font-weight:300; background-color:#e6e6e6;"><br>
      ...
      <tr>
        <td width="580" align="right"><table width="700" border="0" align="right" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="200" align="center" valign="bottom" style="text-align:center; border-top:1px solid #2a2a2a;  border-top:1px solid #a5a5a5; border-bottom:1px solid #a5a5a5; border-left:1px solid #a5a5a5; border-right:1px solid #a5a5a5; font-size:10px; font-weight:700;">'.mb_strtoupper($rfc).'</td>
          </tr>
        </table></td>
      </tr>
      <tr>
      ...
      <tr>
        <td align="left" style="text-align:justify;;"><table width="700" border="0" align="left" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td width="10" height="10" valign="bottom">&nbsp;</td>
            <td width="100" height="10" valign="bottom"  style="color: #565656; font-size: 8px;">CORREO ELECTRÓNICO:</td>
            <td width="150" height="10" align="center" valign="bottom" style="text-align:center;  border-top:1px solid #a5a5a5; border-bottom:1px solid #a5a5a5; border-left:1px solid #a5a5a5; border-right:1px solid #a5a5a5; font-size:10px; font-weight:700;">'.$EmailF.'</td>
          </tr>
          <tr>
      ...
      <tr>
        <td width="700" height="25" align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:10px; font-weight:300; background-color:#e6e6e6;"><br><strong>DATOS DEL ESTABLECIMIENTO Y GIRO</strong></td>
      </tr>
      <tr>
        <td height="10" align="left" style="text-align:justify;;">&nbsp;</td>
      </tr>
      ...
      <tr>
        <td height="25" align="center" valign="middle" style="font-family:Arial, Helvetica, sans-serif; font-size:10px; font-weight:300;  background-color:#e6e6e6;"><br><strong>DATOS DEL INMUEBLE</strong></td>
        </tr>
      <tr>
        <td height="10" align="left" style="text-align:justify;;">&nbsp;</td>
      </tr>
...
$to = $EmailF;
$subject = utf8_decode('Solicitud Licencias de Funcionamiento para Negocios y Empresas');
$from = $emailAdmin;
					...
					<table class="display-width-inner" width="600" cellspacing="0" cellpadding="0" border="0" align="center">
						...
						<tr>
							<td class="MsoNormal txt-center" style="color:#666666; font-family:Segoe UI, Helvetica Neue, Arial, Verdana, Trebuchet MS, sans-serif; font-size:14px; font-weight:400; line-height:24px;" data-color="Content" data-size="Content" data-min="12" data-max="33" align="left">
							  ...
							  <p>Adjunto a este correo encontrá su Formato Único del Sistema de Aperutura Rápida de Empresas SARE, el cual es un comprobante de su solicitud en línea.</p>
							  </td>
						</tr>
						<tr>
							<td height="20">
						...
						<tr>
							<td class="MsoNormal txt-center" style="color:#666666; font-family:Segoe UI, Helvetica Neue, Arial, Verdana, Trebuchet MS, sans-serif; font-size:14px; font-weight:400; line-height:24px;" data-color="Content" data-size="Content" data-min="12" data-max="33" align="left">
							El usuario y contraseña con el que puede acceder al sistema de licencias para descargar todos los formatos de su trámite son los siguientes:<br><br>
							Dirección del sistema: <a href="'.$urlmail.'/acceso.php">'.$urlmail.'</a><br>
							Usuario: '.$EmailF.'<br>
							Contraseña: '.md5($passV).'<br>						
							</td>
						</tr>
						<tr>
							<td height="20">
...
$url = $urlmail.'/plataforma/'.$folder_docs.'/'.$Folio.'/'.$Folio.'-FUA.pdf';
$fichero = file_get_contents($url);
$mail->addStringAttachment($fichero, $Folio.'-FUA.pdf', $encoding, $type);
//Para adjuntar archivo
