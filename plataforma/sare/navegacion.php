										<!-- ===================================================
                                        ================= NAVIGATION Content ===================
                                        ==================================================== -->
                                        <ul id="navigation">
                                            <li class="<?php if ($menu== '0'){ echo 'active'; } else {}	?>">
                                                <a href="./"><i class="fa fa-dashboard"></i> <span>Escritorio</span></a>
                                            </li>
                                            <li class="<?php if ($menu== '1'){ echo 'active'; } else {}	?>">
                                                <a href="agrega_sol.php" tabindex="0"><i class="fa fa-sign-in"></i> <span>Capturar Solicitud</span></a>
                                            </li>
                                            <li class="<?php if ($menu== '2'){ echo 'active'; } else {}	?>">
                                                <a href="citas/" tabindex="1"><i class="fa fa-calendar"></i> <span>Citas Programadas</span></a>
                                            </li>
                                            
                                            <li class="<?php if ($menu== '3'){ echo 'active'; } else {}	?>">
                                                <a href="../salir.php"  tabindex="0"><i class="fa fa-power-off"></i> <span>Salir</span></a>
                                            </li>

                                        </ul>
                                        <!--/ NAVIGATION Content -->